...
fun homeCaseForState(state: ConclusionState): ConclusionCase = when (state) {
...
fun homePrimaryAction(state: ConclusionState): EvidriloHomeAction = when (state) {
...
data class EvidriloHomeObservation(
    val label: String,
    val value: String,
    val factId: String,
)

enum class EvidriloHomeObservationLayout {
    FIT,
    HORIZONTAL_SCROLL,
}

fun homeObservationLayout(observationCount: Int): EvidriloHomeObservationLayout =
...
enum class EvidriloRecommendationSurface {
    NONE,
    LOADING,
    AVAILABLE,
    ACTION_IN_PROGRESS,
...
fun recommendationHomeSurface(state: RecommendationUiState): EvidriloRecommendationSurface = when (state) {
