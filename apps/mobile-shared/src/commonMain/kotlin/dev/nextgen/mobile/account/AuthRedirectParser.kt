...
internal const val MAX_AUTH_REDIRECT_URL_BYTES: Int = 8 * 1024

internal sealed interface AuthRedirect {
    data class Code(
        val code: String,
        val state: String,
    ) : AuthRedirect
...
internal fun parseAuthRedirect(
    rawUrl: String,
...
internal fun isBoundedAuthRedirectUrl(value: String): Boolean =
    value.length <= MAX_AUTH_REDIRECT_URL_BYTES &&
        value.encodeToByteArray().size <= MAX_AUTH_REDIRECT_URL_BYTES

internal fun encodeAuthUrlComponent(value: String): String {
