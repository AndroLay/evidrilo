...
E190 adds the first Evidence Graph presentation slice and closes API rule
anchor edges with deterministic opaque unresolved nodes when the referenced
source is absent. The regression and contract checks pass on `main`; this is
repository/contract-boundary evidence and does not claim mobile runtime,
accessibility-service, provider, managed, human, or submission evidence.
